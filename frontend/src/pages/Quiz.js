...
function Quiz({ section, utilisateur, onRetourDashboard }) {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/questions/section/${section}`)
  ...
  const validerReponse = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.post( `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/ia/expliquer`, {
        question: q.question,
